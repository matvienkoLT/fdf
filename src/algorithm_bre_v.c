...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/07/11 13:01:43 by lmatvien          #+#    #+#             */
/*   Updated: 2018/07/11 13:39:57 by lmatvien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#define X1 block->h_node->x_mod
#define Y1 block->h_node->y_mod
...
static void	dy_direction(t_cord_bre *krd, t_pixel_pack *block, int *sz, int *im)
{
	...
	while (i <= krd->dy)
	{
		...
		filling_grad_color(&im[krd->x + sz[0] * krd->y], block, &krd->grd, &i);
		krd->y += krd->sy;
		i++;
	}
}
...
void		algorithm_bre_v(t_pixel_pack *block, int *im, int *sz, int *gr)
{
	t_cord_bre krd;
